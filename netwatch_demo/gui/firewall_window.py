"""
L.O.G.S. — Firewall Analyzer Window
Presentation-ready window: analysis, rule browsing, and rule management.
"""
...
# Minimal palette (matches main app)
BG   = "#111114";  SURF = "#1a1a1e";  HOV  = "#22222a"
BDR  = "#2a2a32";  TXT  = "#dcdce8";  MUT  = "#666674"
ACC  = "#1565c0";  SUC  = "#3d9e6e";  WAR  = "#b8882e";  DAN = "#b04040"
...
class StatCard(QFrame):
    ...
    clicked = pyqtSignal(str)  # emits filter_key ("High" / "Medium" / "Low" / "")
...
class FirewallDemoWindow(QMainWindow):
    """
    Presentation-ready demo window.

    Layout:
    ┌──────────────────────────────────────────────────────┐
    │  Header  (title + KPI cards)                         │
    ├──────────────────────────────────────────────────────┤
    │  Toolbar  (Analyze · Add Rule · Search | progress)   │
    ├──────────────────────────────────────────────────────┤
    │  Tabs:  Issues  |  All Rules                         │
    ├──────────────────────────────────────────────────────┤
    │  Action bar  (Enable · Disable · Remove)             │
    └──────────────────────────────────────────────────────┘
    """
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("L.O.G.S. — Firewall Analyzer")
    ...
    def _build_tabs(self) -> QTabWidget:
        ...
        self._tabs.addTab(self._build_changes_tab(),   "Recent Changes")
        return self._tabs
    ...
    def _build_changes_tab(self) -> QWidget:
        ...
        hdr = self._changes_table.horizontalHeader()
        hdr.setSectionResizeMode(0, QHeaderView.ResizeToContents)
        hdr.setSectionResizeMode(1, QHeaderView.ResizeToContents)
        hdr.setSectionResizeMode(2, QHeaderView.Stretch)
        hdr.setSectionResizeMode(3, QHeaderView.Stretch)
        lay.addWidget(self._changes_table, stretch=1)

        # Thin footer with Clear button
        footer = QFrame()
    ...
    def _reset_cards(self):
        ...
        self._card_total.set_active(False)
        self._card_high.set_active(False)
        self._card_medium.set_active(False)
        self._card_low.set_active(False)
        for card in (self._card_total, self._card_high,
                     self._card_medium, self._card_low):
            card.set_value("—")
    ...
    def _filter_by_severity(self, severity: str):
        if severity and self._severity_filter == severity:
            severity = ""           # click same filter again → clear it
    ...
    def _selected_rule_name(self) -> str | None:
        ...
        if tab == 0:
            ...
            if row < 0:
                ...
                return None
            return self._displayed_issues[row].get("rule_name") if row < len(self._displayed_issues) else None
        elif tab == 1:
            ...
            if row < 0:
                ...
                return None
            return self._all_rules[row].get("DisplayName") if row < len(self._all_rules) else None
        else:  # Recent Changes tab
            row = self._changes_table.currentRow()
            if row < 0:
                QMessageBox.warning(self, "No Selection",
                                    "Select a row from the Recent Changes table first.")
                return None
            return self._changes[row].get("rule_name") if row < len(self._changes) else None
    ...
    def _log_change(self, action: str, rule_name: str, details: str = ""):
        ...
        self._tabs.setTabText(2, f"Recent Changes  ({count})")
    ...
    def _clear_changes(self):
        ...
        self._tabs.setTabText(2, "Recent Changes")
    ...
    def _run_rule_action(self, title: str, msg: str, method: str, log_action: str):
        """Shared handler for enable/disable/remove — avoids repeating the same pattern."""
        from core.firewall import FirewallAnalyzer
